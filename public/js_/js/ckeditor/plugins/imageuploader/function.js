...
// Open image modal
function showImage(imgSrc, imgStyle, path) {
    var imgSrc = imgSrc;
    ...
    $("#imgActionDelete").attr("onclick","window.location.href = 'imgdelete.php?img=" + path + "'");
    $("#imgActionDownload").attr("href", imgSrc);

}

// Open editbar
function showEditBar(imgSrc, imgStyle, imgID, path) {
    var imgSrc = imgSrc;
    var imgStyle = imgStyle;
    var imgID = imgID;
    ...
    $("#editbarDelete").attr("onclick","deleteImg('" + path + "', '" + imgID + "');");
...
function uploadImg() {
    ...
    if($('#files .fileDiv,#files .fullWidthFileDiv').length < 10 ) {
        $("#uploadImgDiv").show();
        $("#background2").slideDown(250, "swing");
    } else {
        alert('Maximum upload is 10 files allowed.');
    }
}
