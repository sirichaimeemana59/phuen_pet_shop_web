$(function () {
        ...
        $('#addRowBtn').on('click', function (e){
            ...
            var category = '<select name="transaction['+time+'][category]">'+ $('#invoice-category-template select').html() + '</select>';
        	var tRowTmp = [
		    '<tr class="item-row">',
		    '<td><i class="deleteRow fa-trash"></i></td>',
		    '<td><input class="toValidate tDesc form-control input-sm" name="transaction['+time+'][detail]" type="text" maxlength="200" /></td>',
            '<td>'+category+'</td>',
		    '<td><input class="toValidate tQty form-control input-sm" name="transaction['+time+'][quantity]" type="text"  maxlength="15"/></td>',
		    '<td><div class="input-group"><span class="input-group-addon">฿</span>' +
		        '<input class="toValidate tPrice form-control input-sm" name="transaction['+time+'][price]" type="text" maxlength="15"></div></td>'];//.join('');
