            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="panel panel-default" id="panel-lead-list">
                        <div class="row">
                            <div class="col-sm-12 text-right">
                                <button class="btn btn-warning mt-2 mt-xl-0 text-right add-store"><i class="fa fa-newspaper-o"></i>  {!! trans('messages.comment.title') !!}</button>
                        ...
                        <div class="panel-body" id="landing-subject-list">
                            @include('comment.list_comment_element')
                        </div>
                    </div>
                </div>
            </div>
        ...
        $(document).ready(function() {
            ...
            function propertyPage (page) {
                // $('.search-store').on('click', function () {
                    ...
                    $.ajax({
                        url: $('#root-url').val() + '/customer/add/comment',
                        method: 'post',
                        dataType: 'html',
                        data: data,
                        success: function (e) {
            ...
            $('.reset-s-btn').on('click',function () {
                ...
                //propertyPageSale (1);
                window.location.href =$('#root-url').val()+'/customer/add/comment';
            });
            ...
            $('body').on('click','.view-store',function(){
                ...
                $('.v-loading').show();
                $.ajax({
                    url : $('#root-url').val()+'/employee/comment/view',
                    method : 'post',
                    dataType : 'html',
                    data : ({'id':id}),
                    success : function(e){
            ...
            $('body').on('click','.edit-store',function(){
                ...
                $('.v-loading1').show();
                $.ajax({
                    url : $('#root-url').val()+'/employee/comment/edit',
                    method : 'post',
                    dataType : 'html',
                    data : ({'id':id}),
                    success : function(e){
