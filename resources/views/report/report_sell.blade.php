            ...
            <div class="card">
                <div class="card-body">
                    <div class="panel-heading">
                        <h3 class="panel-title">{!! trans('messages.sale') !!}</h3>
                    </div>
...
{{--                                <a href="{!! url('/report/sale_report/print') !!}" target="_blank"><button class="btn btn-success mt-2 mt-xl-0 text-right"><i class="fa fa-file-text"></i>  {!! trans('messages.report_show') !!}</button></a>--}}
                            </div>
                        </div>
                        <br>
                        <div class="panel-body" id="landing-subject-list">
                            @include('report.report_sell_element')
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header" style="background-color: #9BA2AB;">
                    <h4 class="modal-title" style="color: #bbbfc3;">{!! trans('messages.sale') !!}</h4>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            function propertyPage (page) {
                ...
                $.ajax({
                    url: $('#root-url').val() + '/report/sale_report',
                    method: 'post',
                    dataType: 'html',
                    data: data,
                    success: function (e) {
            ...
            $('.reset-s-btn').on('click',function () {
                ...
                //propertyPageSale (1);
                window.location.href =$('#root-url').val()+'/report/sale_report';
            });
            ...
            $('body').on('click','.view-store',function(){
                ...
                $('.v-loading').show();
                $.ajax({
                    url : $('#root-url').val()+'/report/sale_report/view',
                    method : 'post',
                    dataType : 'html',
                    data : ({'id':id}),
                    success : function(e){
